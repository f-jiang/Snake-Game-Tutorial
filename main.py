...
from kivy.properties import ObjectProperty, BooleanProperty, ListProperty, NumericProperty, OptionProperty,\
...
class Playground(Widget):
    ...
    # grid parameters (chosen to respect the 16/9 format)
    col_number = 16
    row_number = 9
    ...
    def update(self, *args):
        ...
        # registering the fruit popping sequence in the event scheduler
        if self.turn_counter == 0:
            self.fruit_rhythm = self.fruit.interval + self.fruit.duration
            Clock.schedule_interval(
                self.fruit.remove, self.fruit_rhythm / self.running_time_coeff)
